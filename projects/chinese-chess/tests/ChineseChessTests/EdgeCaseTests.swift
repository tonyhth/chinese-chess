...
struct EdgeCaseTests {
    ...
    func generalsBlockedByPiece() {
        let rg = Piece(kind: .general, side: .red, position: Position(row: 9, col: 4))
        let bg = Piece(kind: .general, side: .black, position: Position(row: 0, col: 4))
        let blocker = Piece(kind: .soldier, side: .red, position: Position(row: 4, col: 4))
    ...
    func generalsPartiallyBlocked() {
        let rg = Piece(kind: .general, side: .red, position: Position(row: 9, col: 4))
        let bg = Piece(kind: .general, side: .black, position: Position(row: 0, col: 4))
        let p1 = Piece(kind: .soldier, side: .red, position: Position(row: 6, col: 4))
        let p2 = Piece(kind: .soldier, side: .red, position: Position(row: 7, col: 4))
    ...
    func horseBlockedAllDirections() {
        // 马在 (4,4)，八个方向各测试蹩腿
        let horse = Piece(kind: .horse, side: .red, position: Position(row: 4, col: 4))
        ...
        let blockerPieces = blockers.map { Piece(kind: .soldier, side: .red, position: $0) }
    ...
    func elephantBlockedAllEyes() {
        ...
        let blockers: [Piece] = [
            Piece(kind: .soldier, side: .red, position: Position(row: 6, col: 1)),  // 左上眼
            Piece(kind: .soldier, side: .red, position: Position(row: 6, col: 3)),  // 右上眼
            Piece(kind: .soldier, side: .red, position: Position(row: 8, col: 1)),  // 左下眼
            Piece(kind: .soldier, side: .red, position: Position(row: 8, col: 3)),  // 右下眼
        ]
    ...
    func elephantCannotCrossRiver() {
        let elephant = Piece(kind: .elephant, side: .red, position: Position(row: 5, col: 2))
    ...
    func cannonCannotCaptureWithoutMount() {
        let cannon = Piece(kind: .cannon, side: .red, position: Position(row: 7, col: 0))
        let target = Piece(kind: .soldier, side: .black, position: Position(row: 7, col: 3))
    ...
    func cannonCaptureWithOneMount() {
        let cannon = Piece(kind: .cannon, side: .red, position: Position(row: 7, col: 0))
        let mount = Piece(kind: .soldier, side: .red, position: Position(row: 7, col: 2))
        let target = Piece(kind: .soldier, side: .black, position: Position(row: 7, col: 4))
    ...
    func cannonCannotCaptureWithTwoMounts() {
        let cannon = Piece(kind: .cannon, side: .red, position: Position(row: 7, col: 0))
        let mount1 = Piece(kind: .soldier, side: .red, position: Position(row: 7, col: 2))
        let mount2 = Piece(kind: .soldier, side: .red, position: Position(row: 7, col: 3))
    ...
    func cannonVerticalCapture() {
        let cannon = Piece(kind: .cannon, side: .red, position: Position(row: 5, col: 4))
        let mount = Piece(kind: .soldier, side: .red, position: Position(row: 4, col: 4))
        let target = Piece(kind: .soldier, side: .black, position: Position(row: 3, col: 4))
        let board = makeBoard(extra: [cannon, mount, target])
        let moves = MoveValidator.legalMoves(for: cannon, on: board)
    ...
    func soldierCannotRetreatAfterCrossing() {
        let soldier = Piece(kind: .soldier, side: .red, position: Position(row: 4, col: 4))
    ...
    func blackSoldierAfterCrossing() {
        let soldier = Piece(kind: .soldier, side: .black, position: Position(row: 5, col: 4))
        let board = makeBoard(extra: [soldier])
        let moves = MoveValidator.legalMoves(for: soldier, on: board)
        let targets = Set(moves.map { $0.to })
        // 黑卒前进方向是 +row
    ...
    func generalCannotLeavePalace() {
        let rg = Piece(kind: .general, side: .red, position: Position(row: 8, col: 3))
        let board = Board(pieces: [rg, Piece(kind: .general, side: .black, position: Position(row: 0, col: 5))])
    ...
    func advisorCannotLeavePalace() {
        let advisor = Piece(kind: .advisor, side: .red, position: Position(row: 8, col: 4))
    ...
    func chariotCannotMoveBeyondCapture() {
        ...
        let behind = Piece(kind: .soldier, side: .black, position: Position(row: 2, col: 0))
