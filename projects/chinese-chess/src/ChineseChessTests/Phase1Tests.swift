...
struct GameModeTests {
    ...
    func testGameModeValues() {
        ...
        #expect(modes.contains(.singlePlayer))
        #expect(modes.contains(.localPVP))
    }
    ...
    @Test("GameMode Codable roundtrip")
    func testGameModeCodable() {
        for mode in GameMode.allCases {
            let data = try! JSONEncoder().encode(mode)
            let decoded = try! JSONDecoder().decode(GameMode.self, from: data)
            #expect(decoded == mode)
        }
    }
}
