...
### 3.2 Level（关卡定义 — 纯静态，与运行时状态分离）

> **P0-1 & P1-9 修复**：关卡定义（哪些单词属于哪关）是静态数据，运行时状态（是否解锁、星星、最高分）独立存储。单一权威来源：`Word.group` 决定分组，`Level` 只定义名称和元信息。

```swift
/// 静态关卡定义，编译时确定，存入 Bundle JSON
struct LevelDefinition: Identifiable, Codable {
    let id: Int          // 关卡编号 1-25
    let name: String     // 关卡名称，如 "入门小镇"
    let wordGroup: Int   // 对应 Word.group，唯一映射关系
}

/// 运行时关卡状态，持久化到 progress.json
...
376 个单词分成 25 关：前 24 关各 15 个，第 25 关 16 个。关卡线性解锁（通过前一关即可解锁下一关）。解锁判断：`levelId == 1 || levelProgress[levelId - 1].isCompleted`。
...
struct WordProgress: Codable {
    ...
    var correctCount: Int           // 累计答对次数
    var consecutiveCorrect: Int     // **P1-5 新增** 连续答对次数（错词移除用）
    var wrongCount: Int
...
**P1-6 Mastery 更新时机**：每答一题实时更新，不做关卡结束批量处理。理由：杀进程不丢单题级别的进度；且实时更新让错词本始终保持最新。
```

### 3.6 GameSession（游戏会话）

```swift
/// **P0-4 修复**：GameSession 可持久化，杀进程后可恢复进行中的会话
...
**布局**：Phase 1 用简洁列表视图（性能稳定），Phase 3 升级为地图路径（类似糖果传奇）。
...
| 条件 | 心情 | 效果 |
|------|------|------|
| 当天有学习且无待复习错词 | happy | 蛋仔笑脸 + 对话鼓励 |
| 当天有学习但有待复习错词 | normal | 蛋仔普通表情 + 提示复习 |
| 超过 1 天未学习 | sad | 蛋仔垂头 + "好久没见了" |
| 升级 / 过关 / 连击 ≥ 5 | excited | 蛋仔跳跃 + 光效 |

PetMood 纯视觉反馈，不影响游戏机制（不扣分/不加难度），避免焦虑感。
...
- 计时模式，60 秒内消除所有配对
- 用时越短分数越高
- **P1-8 处理（选词范围）**：优先从当前关卡未掌握单词选 8 对，不够则从错词本补充，再不够则从已学单词随机。确保每局都是对玩家有价值的练习。

#### 4.6.4 每日挑战（Daily Challenge）

- 每天 1 组题（从所有已学单词中随机 15 个）
- **P1-7 处理**"已学"定义：`mastery >= .learning`（即至少答过一次）。如果已学单词不足 15 个，从当前可玩关卡的单词中随机补充，避免重复。
- 限时 3 分钟
- 题型混合
- 记录历史最高分
- 完成奖励：金币 + 蛋仔经验
...
- 自动收录答错的单词（`wrongCount > 0` 的词）
- 采用间隔重复算法安排复习（`nextReviewDate <= now` 的优先）
- 优先推近期出错的词
- **P1-5 处理**：用 `consecutiveCorrect >= 3` 判断移除（连续答对 3 次），而非累计值
- 答对时 `consecutiveCorrect += 1`，答错时 `consecutiveCorrect = 0`
...
| 关卡定义 | App Bundle JSON 文件 | 只读，静态数据 |
| 玩家档案 | UserDefaults | 单例，数据量小 |
| 学习进度 + 关卡状态 + 进行中会话 | Documents 目录 JSON 文件 | 可变，原子写入 |
...
```json
{
  "version": 1,
  "wordProgress": {
    "1": {"mastery": 2, "correctCount": 5, "consecutiveCorrect": 2, "wrongCount": 1, "lastReviewed": "...", "nextReviewDate": "..."},
    ...
  },
  "levelProgress": {
    "1": {"isCompleted": true, "stars": 3, "bestScore": 950},
    ...
  },
  "activeSession": {
    "id": "...",
...
写入流程：
1. 序列化 JSON 到临时文件 `progress.tmp.json`
2. 调用 `FileManager.replaceItem(at:withItemAt:)` 原子替换
3. 如果替换失败，保留原文件不变

恢复流程：
1. 启动时读取 `progress.json`
2. 如果文件不存在或解析失败，检查 `progress.json.bak`（上一次成功写入的备份）
3. 都失败则从零开始（新用户体验）
...
376 条数据全扫描成本可忽略（< 1ms），但为了语义清晰，Repository 提供专门方法：
```swift
func getWordsDueForReview(now: Date) -> [WordProgress] {
    allProgress.values.filter { $0.nextReviewDate <= now && $0.mastery != .mastered }
}
```
不做额外索引结构，376 条够小。
...
| 场景 | 处理方式 |
|------|----------|
| JSON 文件解析失败 | 降级到默认数据（新用户体验），Log 错误 |
| 写入失败 | 保留内存中的最新状态，下次写入时重试。不 crash。 |
| 音效文件缺失 | 静默跳过，只打 Log。不影响游戏流程。 |
| progress.json 损坏 | 从 .bak 恢复，都失败则重置（新用户体验） |
| TTS 不可用 | 听音题型降级为其他题型，不阻塞 |

所有错误用统一的 `AppError` 枚举，Repository 层抛出，ViewModel 层 catch 并决定 UI 反馈。
...
- 音效可全局开关（设置页）
- 预加载常用音效，避免延迟
- **P2-17**：progress.json 放 Documents 目录，默认会被 iCloud 备份。如不需要同步，可设置 `URL.resourceValues(forKeys: [.isExcludedFromBackupKey])` 排除。当前方案不计划 iCloud 同步。

### 8.3 TTS（发音）

使用系统内置 `AVSpeechSynthesizer` 朗读单词，无需第三方服务。听音选词题型直接用 TTS。

**P2-14 处理**：Phase 1 先验证 TTS 对这 376 个单词的发音质量（大部分是常见词，问题不大）。如遇发音不准的词，可在 wordlist.json 中加 `phonetic` 字段手动修正。

**P2-15 处理（AudioService 预加载）**：Phase 1 用简单的懒加载 + 缓存策略。首次播放后缓存 AVAudioPlayer 实例，后续复用。实测延迟可接受则不改。
...
答对时：
  ...
  - 如果 consecutiveCorrect >= 3 且在错词本中 → 从错词本移除
...
**P1-10 连续天数时区安全处理**：所有日期比较使用 `Calendar.current.isDateInToday()`、`Calendar.current.dateComponents([.day], from: lastDate, to: today).day!` 等本地日历 API，不用 UTC timestamp 直接比较。
...
Phase 1 用 **纯 SwiftUI 绘制** 蛋仔（圆形身体 + 眼睛 + 腮红 + 四肢），配合 `@State` 动画实现 4 种表情变化。后期如有美术资源可替换。

5 级形态变化推迟到 Phase 3（**P2-13**）。Phase 1 的蛋仔通过颜色/配饰微调来区分等级，不需要完全重画。
...
| 任务 | 说明 |
|------|------|
| 项目搭建 | Xcode 项目（**含 macOS target，P2-18**）、目录结构、构建脚本 |
| 数据层 | Models + WordRepository + JSON 数据 |
| 持久化 | ProgressRepository（原子写入 + 备份恢复） |
| 闯关模式 | 关卡选择 → 答题（选释义 + 选单词）→ 结算 |
| 会话保存 | GameSession 持久化，杀进程可恢复 |
| 蛋仔展示 | SwiftUI 绘制基础蛋仔 + 4 种表情，答对弹跳、答错摇头 |
| 基础 UI | MainTabView + 关卡列表（简化版，不做路径地图） |
| 错误处理 | AppError 枚举 + 各场景降级策略 |
...
| 每日挑战 | 随机题目 + 计时 |
| 错词本 | 自动收录 + consecutiveCorrect 计数 + 间隔重复复习 |
| 金币系统 | **P1-11**：基础产出和消费闭环 |

**金币经济设计（P1-11）**：
- 获取：过关 +50、每日挑战 +30、连续打卡 bonus（3天/7天/30天递增）
- 消费：蛋仔装饰（50-200 金币/件，5-8 件基础装饰）
- 不做付费，金币是纯游戏内循环
...
| 打卡日历 | 连续天数记录 + 可视化 |
| 关卡主题 | 25 关的名称和背景差异化 |
| 蛋仔 5 级形态 | 各等级视觉差异化（美术资源 or SwiftUI 进阶绘制） |
| macOS 适配 | 窗口大小适配、键盘快捷键 |
| 数据备份 | iCloud 同步或手动导出（P2-20） |
...
| 风险 | 影响 | 应对 |
|------|------|------|
| 蛋仔形象资源缺失 | 视觉效果打折 | Phase 1 用 SwiftUI 纯绘制 1 个形态 + 4 表情，Phase 3 扩展 5 级形态 |
...
| 数据损坏 | 进度丢失 | 每次写入前备份 .bak，损坏时自动从备份恢复（P0-2 已设计） |

---

## 14. 测试要点

**P2-16 构建脚本触发时机**：`generate_wordlist.py` 作为 Xcode Build Phase Run Script 自动执行（仅在 `wordlist.md` 更新时重新生成）。脚本检测 `wordlist.json` 的 mtime 与 `wordlist.md` 比较决定是否重新生成。
...
1. **蛋仔美术资源**：Phase 1 用 SwiftUI 纯代码绘制基础形态。如果洪涛后续提供美术素材，Phase 3 替换。当前不影响开发。
2. **音效来源**：Phase 1 用系统音效（`AudioServicesPlaySystemSound`），Phase 2 替换为免费音效库精选音效。
3. **macOS 最低版本**：macOS 14+（与 iOS 17+ 对齐，SwiftUI 最新特性可用）。Phase 1 建 target 确保编译通过。
4. **所有 P0/P1 已在 v2 方案中修复，不再需要额外确认。**
