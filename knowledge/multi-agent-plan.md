...
| **lead** | Luke·卢克 | 男 | 架构师/调度者：拆解任务、定架构、汇总结果、质量决策 | glm-5.1 | 不写具体代码，负责任务分配、流程调度和质量把关 |
| **coder** | Cody·科迪 | 男 | 编码者：写代码、实现功能 | glm-5.1 | 只负责写代码，不自审。根据 Ruby 清单自行决定是否修复 |
| **reviewer** | Ruby·露比 | 女 | 审查者：输出一次性问题清单，不做复审，不发@ | glm-5.1 | 只读不改，审查完即结束，不发@任何人 |
| **tester** | Tina·蒂娜 | 女 | 测试者：最终质量关卡，测试结果决定代码是否合格 | glm-5-turbo | 只写测试和跑测试，不改产品代码 |
...
**设计原则：单向链式流水线**
- 每个角色做完自己的事就结束，**不回头**
- **链式传递**：每个角色只被唯一上游 @ 一次，无分叉、无并行触发
- Ruby 输出一次性问题清单，**不 @任何人**，Cody 是唯一有权限 @Tina 的人
- Tina 是最终质量关卡：测试通过 = 代码合格，测试不过 = 有 bug
- Luke 根据汇总结果做决策：通过、返工（新一轮流水线）、或有条件接受
- **串行队列**：Luke 同时只处理一个任务，避免群消息上下文混乱
- **无循环、无裁决**。返工不是在原来的循环里打转，而是 Luke 发起新一轮完整流水线
