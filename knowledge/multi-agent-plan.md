...
| **reviewer** | Ruby·露比 | 女 | 审查者：输出一次性问题清单，不做复审 | glm-5.1 | 只读不改，审查完即结束，不等返工 |
| **tester** | Tina·蒂娜 | 女 | 测试者：最终质量关卡，测试结果决定代码是否合格 | glm-5-turbo | 只写测试和跑测试，不改产品代码 |

### 工作流水线（单向流动，无回环）

```
需求 → @Luke拆解 → @Cody写代码 → @Ruby审查 → @Tina测试 → @Luke汇总
                                                                ↓
                                                    测试不过 → Luke判断是否返工
