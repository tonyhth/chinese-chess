...
**需要 Alex + Vera 的任务**（Luke 判断）：
- 新功能/新系统开发
- 多文件重构
- 技术选型决策
- 复杂度中等以上的任务（预估超过 2 天工作量）

**不需要 Alex + Vera 的任务**（直接进入编码）：
- Bug 修复（已知问题、定位明确的）
- 小脚本/工具（单文件、逻辑简单）
- 配置调整/文档更新
- Ruby/Tina 指出的简单修复

**Luke 的判断标准**：这件事需要先想清楚再动手吗？需要 → 走 Alex。不需要 → 直接派 Cody。
...
- **返工控制**：Alex 修改方案最多 2 轮，超过由 Luke 介入裁决
...
- 新增定稿机制：Vera 审查后，Luke 读摘要决定是否定稿（简单）或深入看方案+清单（复杂）
- 新增方案级问题回退：Ruby 摘要中标明方案级问题时，打回给 Alex
- 工作流程更新：
  - 需要方案的任务：→ Alex → Vera →(Alex 修)→ Luke 定稿 → Cody → Ruby → Tina → Luke
  - 不需要方案的任务：→ Cody → Ruby → Tina → Luke（不变）
- "你不写代码"规则不变，自然解决了"方案没人写"的问题
