...
Engine::Engine(std::optional<std::string> path) :
    ...
    // P0 fix: pos.set() 延迟到 pikafish_init 中 NNUE 加载后调用
    // 原代码 pos.set(StartFEN, &states->back()) 在此处调用，
    // 但 NNUE 权重可能未加载完成，导致 Position::set_state() 空指针崩溃
    // 现在只初始化为空 Position，pikafish_init 会调 engine->set_start_position()
...
void Engine::set_start_position() {
