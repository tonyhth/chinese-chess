...
struct TimeManager {
    ...
    static func forDifficulty(_ difficulty: AIDifficulty, isIOS: Bool = false) -> TimeManager? {
        switch difficulty {
        case .beginner, .easy, .medium:
            return nil
        case .hard:
            return TimeManager(timeLimitMs: isIOS ? 3000 : 5000, startTime: Date())
        case .master:
            return TimeManager(timeLimitMs: isIOS ? 5000 : 10000, startTime: Date())
        }
    }
}
