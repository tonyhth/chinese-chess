...
func runCMAESFromCLI() async {
    ...
    // Intel Mac 优化:使用 medium 难度(比 hard 快约 3-5 倍)
    ...
    print("  CMA-ES 自动调参")
    fflush(stdout)
    print("  种群: \(popSize) | 最大代数: \(maxGen) | 每代评估: \(gamesPerEval)局 | 难度: medium")
    fflush(stdout)
    print("═══════════════════════════════════════════")
    fflush(stdout)
    print("")
    ...
    // 导出最优权重
    let outputURL = URL(fileURLWithPath: "eval-weights.json")
