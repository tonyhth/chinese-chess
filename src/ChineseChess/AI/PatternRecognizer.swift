...
struct PatternRecognizer {
    ...
    enum Pattern: String, CaseIterable {
        ...
        case ironGateVariant       // 铁门栓变体（车+炮/双车同列）
    }
    ...
    static func bonusPatterns(on board: Board, for side: Side) -> Int {
        ...
        // 窝心马：马在自己将/帅的正前方（九宫中心），受困
        if let opGeneral = board.generalPosition(of: opSide) {
            let opHorses = opPieces.filter { $0.kind == .horse }
            for horse in opHorses {
                // 马在对方自己九宫中心（将的正前方一格）
                let localRow = (opSide == .black) ? horse.position.row : (9 - horse.position.row)
                if localRow == 1 && horse.position.col == 4 {
                    bonus += 600  // 对方窝心马，我方有利
                }
            }
        }
        // 检查我方窝心马（减分）
        if let myGeneral = board.generalPosition(of: side) {
            let myHorses = myPieces.filter { $0.kind == .horse }
            for horse in myHorses {
                let localRow = (side == .black) ? horse.position.row : (9 - horse.position.row)
                if localRow == 1 && horse.position.col == 4 {
                    bonus -= 600  // 我方窝心马
                }
            }
        ...
        if myKinds[.chariot, default: 0] >= 1 && myKinds[.cannon, default: 0] >= 1 {
            if let opGeneral = board.generalPosition(of: opSide) {
                ...
                for chariot in chariots {
                    for cannon in cannons {
                        if chariot.position.col == opGeneral.col && cannon.position.col == opGeneral.col {
                            bonus += 1000  // 同列铁门栓变体
                        }
                    }
                }
