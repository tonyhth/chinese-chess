...
struct AIEngineP1bP2Tests {
    ...
    func dynamicValueDoesNotCrashHard() {
        let engine = AIEngine()
        let board = Board()
        let move = engine.bestMove(for: board.snapshot(), difficulty: .hard)
        #expect(move != nil)
    ...
    func dynamicValueDoesNotCrashMaster() {
        let engine = AIEngine()
        let board = Board()
        // 大师级耗时较长，只验证不 crash
    ...
    func dynamicValueEasyNotAffected() {
        let engine = AIEngine()
        let board = Board()
    ...
    func horseJumpTargetsInitialBoard() {
        // 红马初始位置 (9,1) 和 (9,7)
        // 验证 AI 能正常处理马的机动性
        let engine = AIEngine()
        let board = Board()
    ...
    func kingSafetyHorseThreatNoCrash() {
        let engine = AIEngine()
        let board = Board()
        let move = engine.bestMove(for: board.snapshot(), difficulty: .master)
        #expect(move != nil)
    ...
    func allDifficultiesValidMovesWithAllImprovements() {
        let engine = AIEngine()
        let board = Board()
    ...
    func aiMoveBoardStateValid() {
        ...
        if let redMove = redMoves.first {
            ...
            // AI 回应
            let aiMove = engine.bestMove(for: boardCopy.snapshot(), difficulty: .medium)
            #expect(aiMove != nil)
