...
struct P1aRegressionTests {
    ...
    func moveScoreChariotAttackBlocking() {
    ...
    func moveScoreCannonAttackBlocking() {
    ...
    func easyInitialLegalMove() {
        let engine = AIEngine()
        let board = Board()
        guard let move = engine.bestMove(for: board.snapshot(), difficulty: .easy) else {
    ...
    func mediumInitialLegalMove() {
        let engine = AIEngine()
        let board = Board()
        guard let move = engine.bestMove(for: board.snapshot(), difficulty: .medium) else {
    ...
    func hardInitialLegalMove() {
        let engine = AIEngine()
        let board = Board()
        guard let move = engine.bestMove(for: board.snapshot(), difficulty: .hard) else {
    ...
    func masterInitialLegalMove() {
        let engine = AIEngine()
        let board = Board()
        guard let move = engine.bestMove(for: board.snapshot(), difficulty: .master) else {
    ...
    func beginnerHasVariety() {
        let engine = AIEngine()
        let board = Board()
        var moves = Set<String>()
        for _ in 0..<30 {
            if let move = engine.bestMove(for: board.snapshot(), difficulty: .beginner) {
