...
struct P1aRegressionTests {
    ...
    func checkmateChariotVsKing() {
        ...
        let result = CheckmateSearch.search(board: LegacySearchBoard(from: board), for: .red, maxDepth: 12, timeLimitMs: 5000)
        // 如果找到将杀，验证走法序列合法性
    ...
    func checkmateFirstMoveIsCheck() {
        ...
        guard let moves = CheckmateSearch.search(board: LegacySearchBoard(from: board), for: .red, maxDepth: 8, timeLimitMs: 3000) else {
