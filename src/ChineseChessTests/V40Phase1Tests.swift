...
struct MoveHangFixTests {
    ...
    func stalemateIsLoss() {
        // 中国象棋：困毙判负，不是和棋
        // v6.2 断言清偿 + m1 TestPieceFactory 合流（Alex 预检 §二域2 同模式）：
        // 坐标取 v6.2 纠错版（旧 fixture 红帅误放 row 9 黑方底线）——红帅(0,4)红宫中心，
        // 出路 (0,3)/(0,5)/(1,3)/(1,4)/(1,5) 全被覆盖且不在将军中；书写取工厂形态
        let rg = TestPieceFactory.redGeneral(0, 4)
        let bg = TestPieceFactory.blackGeneral(9, 4)
        let bc1 = TestPieceFactory.makePiece(kind: .chariot, side: .black, position: Position(row: 2, col: 3))
        let bc2 = TestPieceFactory.makePiece(kind: .chariot, side: .black, position: Position(row: 2, col: 5))
        let bp = TestPieceFactory.makePiece(kind: .soldier, side: .black, position: Position(row: 2, col: 4))
