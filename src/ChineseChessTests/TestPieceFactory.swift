...
//
// 2026-08-29 SIGILL 案根治（v6.3，Eric）：池从 0..63 迁到 64..127。
...
// 迁到 64..127 后与任何合法字面量（0..31）物理隔离，同盘撞 id 从机制上不可能。
//
...
enum TestPieceFactory {
    /// 全局单调 id 池（64..127 区段——与手写字面量 0..31 物理隔离；
    /// 2× 棋子上限轮转余量，单局面内 32 子上限下永不冲突）
    private static let lock = NSLock()
    private static var nextId = 0
    private static let poolSize = 64   // 2× 棋子上限，轮转余量
    private static let poolBase = 64   // 区段基址：永不落入字面量区 0..31
    ...
    static func resetForFreshPosition() {
        lock.lock()
        nextId = 0   // 归零后仍在 64..127 区段（poolBase 偏移），与字面量隔离不变
        lock.unlock()
    }
}
