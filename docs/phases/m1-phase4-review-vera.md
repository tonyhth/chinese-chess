...
### ③ remap 行号抽验（Luke 指定项，后补）——发现实质漂移，非阻塞

**抽验结果**：v1.2 “现状实锚 08-16 晚”（:730/:745/:772/:845/:954/:982/:985 + root :174/:466）相对当前 working tree 已**统一 +32 再漂**：root → :206/:498；超时 :762 / razoring :777 / 叶子 :804 / futility :877 / QS 终止 :986/:1014 / standPat :1017。

**成因**：P3-②/③ 代码已落 working tree（:792 isTerminal 预检删除注记 / :839-843 legalCount==0 甲定案 static eval / :880-884 futility 首着保护 legalCount>0）——行号漂移与 P3 落地同步，属预期编码期漂移。

**判定**：非阻塞。① 文档自带“编码以 grep 现核”对冲；② 语义锚 7/7 现验证健在（shouldStop→evaluate / staticEval / staticEvalForFutility / standPat 逐点确认）；③ 语义不受影响——行漂不改分流表主张。

**处置**：① Cody 开工以本轮新锚为准（上表）；② **Ruby 对照点补一条**：futility 廉价化与 P3-② 首着保护（legalCount>0）将在同一 if 区域叠加——“何时 prune”与“评估取值来源”两轴语义不冲突，但同区域双改动值得审单对照。
